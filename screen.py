...
def save_game(current_board, slot_number):
    # שמירת עותק עצמאי של המטריצה כדי שתזוזות עתידיות לא ישנו את מה שנשמר
    ...
    for row in range(BOARD_ROWS):
        for col in range(BOARD_COLUMNS):
            if board[row][col] == MINE:
                if (
                    col == 0 or board[row][col - 1] != MINE
                ):  # אם אנחנו בעמודה ה 0 אז פשוט מציירים ואם אנחנו הגענו לפצצה אז אנחנו בודקים עם היא שמאלית ביותר
                    mine_x = col * cell_size
                    mine_y = row * cell_size
                    screen.blit(
                        mine_img, (mine_x, mine_y)
                    )  # ומציירים רק את הפצצה השמאלית כי היא באורך של שלוש ולכן צריך רק את השמאלית
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_DOWN:
                    if can_soldier_move(board, "s") == True:
                        do_win = move_soldier(
                            board, "s"
                        )  # פרמטר שמזיז את השחקן ולאחר מכן מקבל תשובה אם הוא ניצח
                        if do_win == False:
                ...
                if event.key in NUMBER_KEYS:  # האם הכפתור שנלחץ הוא מספר
                    number_and_time[event.key] = (
                        time.time()
                    )  # שמירת המספר והזמן שהוא נלחץ
            elif event.type == pygame.KEYUP:  # בודק האם המקש הורם
                if event.key in number_and_time.keys():
                    press_duration = (
                        time.time() - number_and_time.pop(event.key)
                    )  # מחסר את הזמן העכשווי לזמן שהוא נלחץ
                    slot_number = NUMBER_KEYS[
                        event.key
                    ]  # המרה מדויקת למספר הסלוט (1-9)
