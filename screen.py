import random
import time
from consts import BOARD_COLUMNS, BOARD_ROWS, HIGHT, MINE, WIDTH, GUARD, GUARD_ROW
...
# הוספנו את הייבוא של פונקציות התזוזה מהקובץ שלך!
from guard import where_guard_col, move_guard_right, move_guard_left
...
try:
    ...
    guard_img = pygame.Surface((cell_size, cell_size))
    guard_img.fill((255, 0, 0))
...
def show_xray(board, screen, soldier_img, soldier_x, soldier_y, mine_img, guard_img, cell_size):
    screen.fill((0, 0, 0))
...
def run_game(board):
    ...
    # משתנים לשליטה בתזוזת השומר
    last_guard_move = pygame.time.get_ticks()
    guard_dir = "right"
    guard_speed = 300  # השומר יזוז כל 300 מילישניות (אפשר לשנות את המספר כדי לזרז או להאט אותו)

    while running:
        current_time = pygame.time.get_ticks()

        # לוגיקת תזוזת השומר העצמאית
        if current_time - last_guard_move > guard_speed:
            guard_col = where_guard_col(board)
            if guard_col is not None:
                # הפיכת כיוון כשהשומר מגיע לקצה המסך
                ...
                # הזזת השומר בפועל
                if guard_dir == "right":
                    hit_soldier = move_guard_right(board)
                else:
                    hit_soldier = move_guard_left(board)

                # אם השומר פגע בחייל במהלך התזוזה שלו
                if hit_soldier == True:
        ...
        for event in events:
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_DOWN and can_soldier_move(board, "s"):
                    ...
                    move_dir = "w"
                elif event.key == pygame.K_LEFT and can_soldier_move(board, "a"):
                    move_dir = "a"
                elif event.key == pygame.K_RIGHT and can_soldier_move(board, "d"):
                    move_dir = "d"
                ...
                if move_dir:
                    ...
                    show_xray(
                        board, screen, soldier_img, soldier_x, soldier_y, mine_img, guard_img, cell_size
                    )
                    time.sleep(1)
                    pygame.event.clear()
                ...
                if event.key in number_and_time:
                    press_duration = time.time() - number_and_time.pop(event.key)
